...
func _process(_delta: float) -> void:
	if !no_move and _is_dragging:
		...
		position = interp(position, tp, transform_interp_speed, _delta)
		...
		if !no_resize_x:
			size.x = interp(size.x, ts.x, transform_interp_speed, _delta)
		if !no_resize_y:
			size.y = interp(size.y, ts.y, transform_interp_speed, _delta)
	elif !no_snap:
		var window_rect := get_rect()
		var screen_rect := get_viewport_rect()
		...
		current_position = interp(current_position, target_position, bounds_interp_speed, _delta)
		size = interp(size, target_size, bounds_interp_speed, _delta)
		position = current_position
	if _size_animation:
		if _target_size.is_equal_approx(size):
			_size_animation = false
		size = interp(size, _target_size, anim_interp_speed, _delta)


# Framerate-independent interpolation.
func interp(from, to, lambda: float, _delta: float):
	return lerp(from, to, 1.0 - exp(-lambda * _delta))
